...
VIDEO_PTIME = 0.040 #1 / 25  # 25fps — fp16 优化后预期可达
...
class HumanPlayer:
    ...
    def _start(self, track: PlayerStreamTrack) -> None:
        ...
        if self.__thread is None:
            # 如果渲染管线已在外部启动（见 RTCManager.handle_offer），不再重复启动
            if hasattr(self.__container, '_render_started') and self.__container._render_started:
                self.__log_debug("Render already started externally, skipping")
                return
    ...
    def _stop(self, track: PlayerStreamTrack) -> None:
        ...
        if not self.__started and self.__container is not None:
            # 停止外部渲染管线（如果由 RTCManager 启动）
            if hasattr(self.__container, 'stop_render'):
